import { Box, Button, Container, FormControl, IconButton, InputLabel, MenuItem, Modal, Select, TextField, ThemeProvider, Tooltip, Typography } from "@material-ui/core"
...
function multisigAddress() {
	...
	async function generateMultisigAddress() {
		try {
			...
			let mul_address = await BtcPlugin.getMultisigAddress(publicKeys.replace(/\s/g, "").split(","), m, n, network)
	...
	return (
		<ThemeProvider theme={MuiTheme}>
				...
				>
					...
					>
						<TextField
							...
							helperText={focus == "m" && (m <= 0 || m > n) ? "m should be 0 < m <= n" : ""}
							onChange={event => {
								setAddress("")
								setFocus("m")
								setM(event.target.value == "" ? event.target.value : parseInt(event.target.value))
							}}
							value={m}
						/>
						<Typography
						...
						<TextField
							...
							helperText={focus == "n" && (n <= 0 || m > n) ? "m should be 0 < m <= n" : ""}
							onChange={event => {
								setAddress("")
								setFocus("n")
								setN(event.target.value == "" ? event.target.value : parseInt(event.target.value))
							}}
							value={n}
						/>
					</Container>
					...
					<FormControl className={classes.formControl} style={{ flex: 1 }}>
						<TextField
							error={focus == "publicKeys" && (publicKeys == "" || helperTextError)}
							multiline
							...
							value={publicKeys}
							helperText={focus == "publicKeys" && helperTextError != "" ? helperTextError : ""}
							onChange={e => {
								setAddress("")
								setFocus("publicKeys")
			...
			{address != "" ? (
				<Container style={{ marginTop: 40, marginBottom: 40, textAlign: "start" }}>
					{/* Address */}
					...
					>
						<Tooltip title={tooltipText} aria-label={tooltipText} disableHoverListener={address == ""} arrow onClose={() => setTimeout(() => setTooltipText("Copy to clipboard"), 300)}>
							<Typography style={{ color: "#fff", flex: 3 }} variant="body1" onClick={() => copyToClipboard(address)}>
								{address}
							</Typography>
						</Tooltip>
						{/* =============== BTC explorer button=============== */}
						<Tooltip title={"Open in BTC Explorer"} aria-label={"Open in BTC Explorer"} arrow>
							<IconButton style={{ color: "#fff", flex: 1 }} onClick={() => window.open(`${config.blockchain_explorer_host}/btc/address/${address}`)}>
								<OpenInNewIcon />
							</IconButton>
						</Tooltip>
						{/*  =============== QR code buttion =============== */}
